#include "../../../quentin/uavs/divided/UAVBeta.h"
...
Define_Module(UAVBeta);

UAVBeta::UAVBeta(){ nextMoveIsWait = false; }

void UAVBeta::initialize(int stage) {
...
void UAVBeta::setTargetPosition() {
    if (nextMoveIsWait) {
...
double UAVBeta::getMaxSpeed() const {
    return speedParameter->isExpression() ? NaN : speedParameter->doubleValue();
}

J UAVBeta::getBattery(){
...
void UAVBeta::rescueDataAndStoreVariables(){
...
Coord UAVBeta::flyAroundSquare(int j){
    Coord c;
    if(waypoints == 0 || waypoints == 4){
        if(waypoints == 4)
            tasks[j].setStatus(2);
...
void UAVBeta::executeTask(int j){
